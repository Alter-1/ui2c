/*
  twiz.c - ZeroCopy TWI/I2C library for Wiring & Arduino

  derived from Arduino TWI library
  ...
  Modified 2023 by Alexandr Telyiatnikov (alter@alter.org.ua) to implement single buffer for rx/tx and extend its size
*/
...
static uint8_t twi_mainBuffer[TWI_BUFFER_LENGTH];
static uint8_t X_TWI_BUFFER_LENGTH = TWI_BUFFER_LENGTH;
uint8_t twi_bUserBuffer = 0;

static uint8_t* twi_masterBuffer = &twi_mainBuffer[0];
static volatile uint8_t twi_masterBufferIndex;
static volatile uint8_t twi_masterBufferLength;

static uint8_t* twi_txBuffer = &twi_mainBuffer[0];
static volatile uint8_t twi_txBufferIndex;
static volatile uint8_t twi_txBufferLength;

static uint8_t* twi_rxBuffer = &twi_mainBuffer[0];
...
void twi_setUserBuffer(uint8_t* uBuffer, uint8_t uLength)
{
  if(uBuffer && uLength) {
    twi_bUserBuffer = true;
    X_TWI_BUFFER_LENGTH = uLength;
    twi_masterBuffer = uBuffer;
    twi_txBuffer = uBuffer;
    twi_rxBuffer = uBuffer;
  } else {
    twi_bUserBuffer = false;
    X_TWI_BUFFER_LENGTH = TWI_BUFFER_LENGTH;
    twi_masterBuffer = &twi_mainBuffer[0];
    twi_txBuffer = &twi_mainBuffer[0];
    twi_rxBuffer = &twi_mainBuffer[0];
  }
}
...
uint8_t twi_writeTo(uint8_t address, uint8_t* data, uint8_t length, uint8_t wait, uint8_t sendStop)
{
  ...
  if(!twi_bUserBuffer) // handle ZeroCopy
  {
    for(i = 0; i < length; ++i){
...
uint8_t twi_transmit(const uint8_t* data, uint8_t length)
{
  ...
  // ensure data will fit into buffer
  if(X_TWI_BUFFER_LENGTH < (twi_txBufferLength+length)){
    return 1;
  }
...
ISR(TWI_vect)
{
  switch(TW_STATUS){
    ...
    case TW_SR_GCALL_DATA_ACK: // data received generally, returned ack
      ...
      if(twi_rxBufferIndex < X_TWI_BUFFER_LENGTH){
